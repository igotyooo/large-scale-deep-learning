...
function task:defineModel(  )
	...
	extm:cuda(  )
	-- Create context-motion concatenator.
	local addm = nn.Sequential(  )
	addm:add( nn.ConcatTable(  ):add( nn.Identity(  ) ):add( extm ) ) -- Concat order depends on getBatch*()!
	addm:add( nn.JoinTable( 1 ) )
...
function task:getBatchTrain(  )
	...
	local labelContext = torch.LongTensor( batchSize )
	...
	for v = 1, numVideoToSample do
		...
		for f = 1, seqLength do
			...
			labelContext[ fcnt ] = cid
		end
	end
	local labelMotion = torch.LongTensor( batchSize )
	labelMotion:copy( labelContext )
	labelMotion = labelMotion:reshape( numVideoToSample, seqLength ):narrow( 2, 1, seqLength - 1 )
	labelMotion = labelMotion:reshape( ( seqLength - 1 ) * numVideoToSample )
	local label = torch.cat( labelContext, labelMotion )
	return input, label
end
...
function task:evalBatch( fid2out, fid2label )
	...
	assert( fid2out[ 1 ]:size( 1 ) == ( seqLength * 2 - 1 ) * numVideo )
	assert( fid2out[ 1 ]:size( 1 ) == fid2label:numel(  ) )
	assert( numOut == self.opt.numOut )
	for oid, out in pairs( fid2out ) do
		local top1 = 0
		for v = 1, numVideo do
			local fsc = ( v - 1 ) * ( seqLength - 0 ) + 1
			local fsm = ( v - 1 ) * ( seqLength - 1 ) + 1 + batchSize
			local fec = fsc + ( seqLength - 0 ) - 1
			local fem = fsm + ( seqLength - 1 ) - 1
			local outContext = out[ { { fsc, fec }, {  } } ]
			local outMotion = out[ { { fsm, fem }, {  } } ]
			assert( fid2label[ fsc ] == fid2label[ fec ] )
			assert( fid2label[ fec ] == fid2label[ fsm ] )
			assert( fid2label[ fsm ] == fid2label[ fem ] )
			local outBoth = torch.cat( outContext, outMotion, 1 )
			local cid2score
			if pooling == 'sum' then
			...
			if rank2cid[ 1 ][ 1 ] == fid2label[ fsc ] then
				top1 = top1 + 1
			end
		end
