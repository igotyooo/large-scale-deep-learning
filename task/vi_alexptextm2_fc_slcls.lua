...
function task:parseOption( arg )
	...
	cmd:option( '-epochSize', 4768, 'Number of batches per epoch.' )
...
function task:defineModel(  )
	...
	-- Create per-scale differentiators.
	local diff1 = nn.Sequential(  )
	diff1:add( nn.ConcatTable(  ):add( nn.Narrow( 2, 2, seqLength - 1 ) ):add( nn.Narrow( 2, 1, seqLength - 1 ) ) )
	diff1:add( nn.CSubTable(  ) )
	diff1:add( nn.Reshape( numVideo * ( seqLength - 1 ), 96, 54, 54 ) )
	local diff2 = nn.Sequential(  )
	diff2:add( nn.ConcatTable(  ):add( nn.Narrow( 2, 3, seqLength - 2 ) ):add( nn.Narrow( 2, 1, seqLength - 2 ) ) )
	diff2:add( nn.CSubTable(  ) )
	diff2:add( nn.Reshape( numVideo * ( seqLength - 2 ), 96, 54, 54 ) )
	-- Create differentiator.
	local diff = nn.Sequential(  )
	diff:add( nn.Reshape( numVideo, seqLength, 96 * 54 * 54 ) )
	diff:add( nn.ConcatTable(  ):add( diff1 ):add( diff2 ) )
	diff:add( nn.JoinTable( 1 ) )	
	diff:cuda(  )
...
function task:getBatchTrain(  )
	...
	local label1 = torch.LongTensor( numVideoToSample, seqLength - 1 )
	local label2 = torch.LongTensor( numVideoToSample, seqLength - 2 )
	local fcnt = 0
	...
	local label = torch.cat( label1:view( -1 ), label2:view( -1 ) )
	return input, label
end
...
function task:evalBatch( fid2out, fid2label )
	...
	assert( fid2out[ 1 ]:size( 1 ) == ( seqLength - 1 + seqLength - 2 ) * numVideo )
	assert( fid2out[ 1 ]:size( 1 ) == fid2label:numel(  ) )
	assert( numOut == self.opt.numOut )
	for oid, out in pairs( fid2out ) do
		local top1 = 0
		for v = 1, numVideo do
			local fs1 = ( v - 1 ) * ( seqLength - 1 ) + 1
			local fs2 = ( v - 1 ) * ( seqLength - 2 ) + 1 + numVideo * ( seqLength - 1 )
			local fe1 = fs1 + ( seqLength - 1 ) - 1
			local fe2 = fs2 + ( seqLength - 2 ) - 1
			local out1 = out[ { { fs1, fe1 }, {  } } ]
			local out2 = out[ { { fs2, fe2 }, {  } } ]
			assert( fid2label[ fs1 ] == fid2label[ fe1 ] )
			assert( fid2label[ fe1 ] == fid2label[ fs2 ] )
			assert( fid2label[ fs2 ] == fid2label[ fe2 ] )
			local outBoth = torch.cat( out1, out2, 1 )
			local cid2score
			if pooling == 'sum' then
				cid2score = outBoth:mean( 1 )
			elseif pooling == 'max' then
				cid2score = outBoth:max( 1 )
			end
			local _, rank2cid = cid2score:float(  ):sort( 2, true )
			if rank2cid[ 1 ][ 1 ] == fid2label[ fs1 ] then
				top1 = top1 + 1
			end
		end
