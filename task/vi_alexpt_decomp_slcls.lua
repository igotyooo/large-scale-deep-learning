...
function task:parseOption( arg )
	...
	cmd:option( '-learnRate', '0,1e-3', 'Supports multi-lr for multi-module like "lr1,lr2,lr3".' )
...
function task:defineModel(  )
	...
	local context = nn.Sequential(  )
	context:add( nn.Reshape( numVideo, seqLength, featSize ) )
	context:add( nn.Mean( 2 ) )
	context:cuda(  )
	-- Create context upsampler.
	-- In:  numVideo, featSize
	-- Out: ( numVideo X seqLength ), featSize
	local contextUp = nn.Sequential(  )
	contextUp:add( nn.Replicate( seqLength, 2 ) )
	contextUp:add( nn.Reshape( numVideo * seqLength, featSize ) )
	contextUp:cuda(  )
	-- Create motion.
	-- In1: ( numVideo X seqLength ), featSize -- Features.
	-- In2: ( numVideo X seqLength ), featSize -- Contexts.
	-- Out: numVideo, ( seqLength * featSize ) 
	local motion = nn.Sequential(  )
	motion:add( nn.CSubTable(  ) )
	motion:add( nn.View( -1, seqLength * featSize ) )
	motion:cuda(  )
	-- Create decompose.
	-- In:  ( numVideo X seqLength ), featSize
	-- out: numVideo, ( featSize + featSize X seqLength )
	local h_feature = nn.Identity(  )(  )
	local h_context = context( h_feature )
	...
	decompose:cuda(  )
	-- Create classifier.
	-- In:  numVideo, ( featSize + featSize X seqLength )
	-- Out: numVideo, numClass
	local classifier = nn.Sequential(  )
	classifier:add( nn.Linear( featSize + featSize * seqLength, numCls ) )
