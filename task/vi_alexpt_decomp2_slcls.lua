...
function task:defineModel(  )
	...
	motion:add( nn.Reshape( numVideo, seqLength, featSize ) )
	motion:add( nn.SplitTable( 2 ) )
	motion:add( nn.CSubTable(  ) )
	motion:add( nn.MulConstant( -1 ) )
	motion:cuda(  )
	-- Create decompose.
	-- In:  ( numVideo X seqLength ), featSize
	-- out: numVideo, ( 2 X featSize )
	local decompose = nn.Sequential(  )
	decompose:add( nn.ConcatTable(  ):add( context ):add( motion ) )
	decompose:add( nn.JoinTable( 2 ) )
	decompose:cuda(  )
	-- Create classifier.
	-- In:  numVideo, ( 2 X featSize )
	-- Out: numVideo, numClass
	local classifier = nn.Sequential(  )
	classifier:add( nn.Linear( 2 * featSize, numCls ) )
