...
function task:parseOption( arg )
	...
	cmd:option( '-learnRate', '0,1e-3,1e-3,1e-3', 'Supports multi-lr for multi-module like "lr1,lr2,lr3".' )
...
function task:defineModel(  )
	...
	agent:add( nn.Linear( seqLength * featSize, seqLength ) ) -- numVideo, seqLength
	agent:add( nn.Sigmoid(  ) )
	agent:add( nn.Reshape( numVideo * seqLength ) )
	agent:add( nn.Replicate( hiddenSize, 2 ) )
	agent:cuda(  )
	-- Join FC and agent, and sum them.
	-- In:  ( numVideo X seqLength ), featSize
	-- Out: numVideo, hiddenSize
	local fcagent = nn.Sequential(  )
	fcagent:add( nn.ConcatTable(  ):add( fc ):add( agent ) )
	fcagent:add( nn.CMulTable(  ) )
	fcagent:add( nn.Reshape( numVideo, seqLength, hiddenSize ) )
	fcagent:add( nn.Mean( 2 ) )
	fcagent:add( nn.Dropout( 0.5 ) )
	agent:cuda(  )
	-- Create FC classifier.
	-- In:  numVideo, hiddenSize
...
function task:groupParams( model )
	...
	params[ 3 ], grads[ 3 ] = model.modules[ 2 ].modules[ 1 ].modules[ 2 ]:getParameters(  ) -- Agent.
	params[ 4 ], grads[ 4 ] = model.modules[ 3 ]:getParameters(  ) -- Classifier.
	optims[ 1 ] = { -- Features.
	...
	optims[ 4 ] = { -- Classifier.
		learningRate = self.opt.learnRate[ 4 ],
		learningRateDecay = 0.0,
		momentum = self.opt.momentum,
		dampening = 0.0,
		weightDecay = self.opt.weightDecay 
