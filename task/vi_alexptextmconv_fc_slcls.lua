...
function task:parseOption( arg )
	...
	cmd:option( '-convSize', 3, 'Convolution size for motion extraction.' )
	-- Train.
...
function task:defineModel(  )
	...
	local convSize = self.opt.convSize
	local diffSize
	local diffLayer
	if diffLevel == 0 then
	...
	diff:add( nn.Reshape( numVideo, 1, seqLength, diffSize:prod(  ) ) )
	diff:add( nn.SpatialConvolution( 1, 1, 1, convSize, 1, 1, 0, 0 ) )
	diff:add( nn.Reshape( numVideo * ( seqLength - convSize + 1 ), diffSize[ 1 ], diffSize[ 2 ], diffSize[ 3 ] ) )
	diff:cuda(  )
	...
	assert( convSize <= seqLength and convSize > 0 )
...
function task:getBatchTrain(  )
	...
	local convSize = self.opt.convSize
	local label = torch.LongTensor( numVideoToSample, seqLength - convSize + 1 )
	local fcnt = 0
...
function task:evalBatch( fid2out, fid2label )
	...
	local batchSize = self.opt.batchSize
	local convSize = self.opt.convSize
	local numVideo = fid2out[ 1 ]:size( 1 ) / ( seqLength - convSize + 1 )
	local pooling = self.opt.videoPool
	local oid2eval = torch.Tensor( numOut ):fill( 0 )
	assert( batchSize == fid2label:numel(  ) + numVideo * ( convSize - 1 ) )
	...
	for oid, out in pairs( fid2out ) do
		...
		local top1 = 0
		for v = 1, numVideo do
			local fs = ( v - 1 ) * ( seqLength - convSize + 1 ) + 1
			local fe = fs + ( seqLength - convSize + 1 ) - 1
