...
function task:defineModel(  )
	...
	diff1:add( nn.Reshape( numVideo, seqLength, diffSize:prod(  ) ) )
	diff1:add( nn.ConcatTable(  ):add( nn.Narrow( 2, 1 + diffScale * 2, seqLength - diffScale * 2 ) ):add( nn.Narrow( 2, 1 + diffScale, seqLength - diffScale * 2 ) ) )
	diff1:add( nn.CSubTable(  ) )
	local diff2 = nn.Sequential(  )
	diff2:add( nn.Reshape( numVideo, seqLength, diffSize:prod(  ) ) )
	diff2:add( nn.ConcatTable(  ):add( nn.Narrow( 2, 1 + diffScale * 2, seqLength - diffScale * 2 ) ):add( nn.Narrow( 2, 1, seqLength - diffScale * 2 ) ) )
	diff2:add( nn.CSubTable(  ) )
	local diff = nn.Sequential(  )	
	diff:add( nn.ConcatTable(  ):add( diff1 ):add( diff2 ) )
	diff:add( nn.CAddTable(  ) ) -- diff:add( nn.CMaxTable(  ) )
	diff:add( nn.Reshape( numVideo * ( seqLength - diffScale * 2 ), diffSize[ 1 ], diffSize[ 2 ], diffSize[ 3 ] ) )
...
function task:getBatchTrain(  )
	...
	local diffScale = self.opt.diffScale
	local label = torch.LongTensor( numVideoToSample, seqLength - diffScale * 2 )
	local fcnt = 0
...
function task:evalBatch( fid2out, fid2label )
	...
	local batchSize = self.opt.batchSize
	local diffScale = self.opt.diffScale
	local numVideo = fid2out[ 1 ]:size( 1 ) / ( seqLength - diffScale * 2 )
	local pooling = self.opt.videoPool
	local oid2eval = torch.Tensor( numOut ):fill( 0 )
	assert( batchSize == fid2label:numel(  ) + numVideo * diffScale * 2 )
	...
	for oid, out in pairs( fid2out ) do
		...
		local top1 = 0
		for v = 1, numVideo do
			local fs = ( v - 1 ) * ( seqLength - diffScale * 2 ) + 1
			local fe = fs + ( seqLength - diffScale * 2 ) - 1
