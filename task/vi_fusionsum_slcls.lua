...
function task:parseOption( arg )
	...
	cmd:option( '-weight', 1, 'Weight applied for the first model.' )
	-- Train.
	...
	cmd:option( '-learnRate', '0', 'Supports multi-lr for multi-module like "lr1,lr2,lr3".' )
...
function task:defineModel(  )
	...
	local weight = self.opt.weight
	-- Manipulate context net.
	local pathNet1 = '/home/dgyoo/workspace/dataout/large-scale-deep-learning/UCF101/vi_alexpt_fc_slcls/model_006.t7'
	self:print( 'Load pre-trained model 1.' )
	local net1 = loadDataParallel( pathNet1, self.opt.numGpu, self.opt.backend )
	self:print( 'Done.' )
	local numModule1 = #net1.modules
	--net1.modules[ numModule1 ]:remove(  ) -- Remove log-softmax.
	--net1.modules[ numModule1 ]:add( nn.SoftMax(  ) )
	net1.modules[ numModule1 ]:add( nn.View( numVideo, -1, numClass ) )
	net1.modules[ numModule1 ]:add( nn.Mean( 2 ) )
	net1.modules[ numModule1 ]:add( nn.MulConstant( weight ) )
	-- Manipulate motion net.
	local pathNet2 = '/home/dgyoo/workspace/dataout/large-scale-deep-learning/UCF101/vi_alexptextm_fc_slcls/model_008.t7'
	self:print( 'Load pre-trained model 2.' )
	local net2 = loadDataParallel( pathNet2, self.opt.numGpu, self.opt.backend )
	self:print( 'Done.' )
	local numModule2 = #net2.modules
	--net2.modules[ numModule2 ]:remove(  ) -- Remove log-softmax.
	--net2.modules[ numModule2 ]:add( nn.SoftMax(  ) )
	net2.modules[ numModule2 ]:add( nn.View( numVideo, -1, numClass ) )
	net2.modules[ numModule2 ]:add( nn.Mean( 2 ) )
	-- Combine context and motion.
	local model = nn.Sequential(  )
	model:add( nn.ConcatTable(  ):add( net1 ):add( net2 ) )
...
function task:groupParams( model )
	...
	params[ 1 ], grads[ 1 ] = model:getParameters(  )
	optims[ 1 ] = {
		learningRate = self.opt.learnRate,
