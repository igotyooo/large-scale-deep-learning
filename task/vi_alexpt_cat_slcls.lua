...
function task:defineModel(  )
	...
	-- Out: numVideo, ( seqLength X featSize )
	local cat = nn.Sequential(  )
	cat:add( nn.View( -1, seqLength * featSize ) ) -- numVideo, ( seqLength X featSize )
	cat:cuda(  )
	-- Create classifier.
	-- In:  numVideo, ( seqLength X featSize )
	-- Out: numVideo, numClass
	local classifier = nn.Sequential(  )
	classifier:add( nn.Linear( seqLength * featSize, numClass ) )
