...
function task:parseOption( arg )
	...
	cmd:option( '-learnRate', '0,16e-5,1e-3', 'Supports multi-lr for multi-module like "lr1,lr2,lr3".' )
...
function task:defineModel(  )
	...
	-- Out: ( numVideo X ( seqLength - 1 ) ), featSize
	local extm = nn.Sequential(  )
	extm:add( nn.Reshape( numVideo, seqLength, featSize ) )
...
function task:groupParams( model )
	...
	params[ 2 ], grads[ 2 ] = model.modules[ 3 ]:getParameters(  ) -- LSTM.
	params[ 3 ], grads[ 3 ] = model.modules[ 4 ]:getParameters(  ) -- Classifier.
	optims[ 1 ] = { -- Features.
